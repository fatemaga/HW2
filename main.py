...
import json
import time
from data_utils import get_vocab, get_datasets
...
def main():
    ...
    if not os.path.exists(PREPROCESSED_FILE_PATH):
        print('No preprocessed images found. Prepossessing. It will take ~hour')
        preprocess_images()
    ...
    if start_epoch:
        if os.path.exists('model' + str(start_epoch-1) + '.pkl'):
            model.load_state_dict(torch.load('model' + str(start_epoch-1) + '.pkl',
    ...
    for epoch in range(start_epoch, start_epoch + NUM_EPOCHS):
        ...
        for batch_sample in train_loader:
            ...
            with torch.set_grad_enabled(True):
                ...
                total_error += error
            if batch_idx % 10 == 0:
                print('epoch', epoch, time.time()-epoch_start, 'training', batch_idx, 'batch_loss', loss.item(),
                      'last_error', error)
            batch_idx += 1
        ...
        for batch_sample in val_loader:
            ...
            if batch_idx % 10 == 0:
                print(time.time()-epoch_start, 'testing on val', batch_idx, 'epoch', epoch, 'last loss', loss.item(),
                      'last error', error)
