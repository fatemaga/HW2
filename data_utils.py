...
class VqaDataset(data.Dataset):
    def __init__(self, vocabs, train=True):
        ...
        features_file = h5py.File(PREPROCESSED_FILE_PATH, 'r')
        coco_ids = features_file['ids'][()]
        self.coco_id_to_index = {id: i for i, id in enumerate(coco_ids)}

        with open(questions_path(train), 'r') as fd:
            questions_json = json.load(fd)
        filtered_questions = list(filter(lambda x: x['image_id'] in coco_ids, questions_json['questions']))
        questions_json['questions'] = filtered_questions

        for inner_index, q in enumerate(prepare_questions(questions_json)):
            vec = torch.zeros(self.q_length).long()
            iid = questions_json['questions'][inner_index]['image_id']
            if iid in self.coco_id_to_index.keys():
                self.iids[inner_index] = iid
                for i, token in enumerate(q):
                    index = self.question_vocab.get(token, 0)
                    vec[i] = index
                self.questions[inner_index] = vec
        del questions_json

        with open(answers_path(train), 'r') as fd:
            answers_json = json.load(fd)
        filtered_answers = list(filter(lambda x: x['image_id'] in coco_ids, answers_json['annotations']))
        answers_json['annotations'] = filtered_answers
        ...
        for i, a_dict in enumerate(answers_json['annotations']):
            ...
            if index is not None:
                ...
                self.answers.append([self.answer_vocab[ans['answer']] for ans in a_dict['answers'] if
                                     ans['answer'] in self.answer_vocab])
            else:
                if train:
                    ...
                    self.answers.append([])
        for i in to_remove:
            self.questions.pop(i, None)
            self.iids.pop(i, None)

        del answers_json

    def __getitem__(self, item):
        iid = self.iids.get(item)
        ...
        if iid is not None:
            ...
            result = {'question': None, 'answer': None, 'image': None, 'item_no': None}
        return result
...
def get_vocab():
    if not os.path.exists(VOCABULARY_PATH):
        ...
        features_file = h5py.File(PREPROCESSED_FILE_PATH, 'r')
        coco_ids = features_file['ids'][()]
        filtered_questions = list(filter(lambda x: x['image_id'] in coco_ids, questions['questions']))
        questions['questions'] = filtered_questions
        filtered_answers = list(filter(lambda x: x['image_id'] in coco_ids, answers['annotations']))
        answers['annotations'] = filtered_answers
        ...
        top_answers = [x[0] for x in sorted(counts.items(), key=lambda x: (x[1], x[0]), reverse=True)][
                      :ANSWER_VOCAB_LIMIT]
